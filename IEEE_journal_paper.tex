...
\maketitle
\begin{abstract}
A feedback based approach for array signal processing is introduced. In the proposed scheme, a signal is continuously re-transmitted between the array and the target of interest.
The feedback based architecture is suggested as a solution to the seeking of spatial domain equivalent of temporal infinite impulse response.
It practically achieves virtually bigger array aperture, enables the extraction of more spatial information about the target and results in high spatial performance of the array.
Led by traditional array performance analysis, we analyze this system in the context of beamforming, covering beamwidth, peak to side-lobe ratio and array directivity. 
We show significant improvement in all aspects and discuss the conditions that lead to enhanced performance.
Performance analysis also predicts infinite improvement in ideal scenarios.
Finally, an application of low sensitivity to estimation errors or noise is presented and analyzed, describing a practical and robust implementation of the feedback based localization concept. 
\end{abstract}
\begin{IEEEkeywords}
...
\input{./Modules/IEEE_journal_paper_IntegrateFeedback.tex}
...
\]
where $R_{\text{rt}}$ is the round-trip distance between the array and the target of interest, we express the range errors as
\[
\Delta R_{\text{rt}}=\frac{\dPhi \lambda}{2\pi}. 
\]
We then show in Fig.~\ref{fig_rangError} that even minor range errors of $\Delta R_{\text{rt}}\sim0.1\lambda$ significantly distort the beampattern.
\begin{figure}[t!]
    \begin{center}
...
Feedback integration in traditional beamformers proved to achieve the spatial domain equivalent of the temporal IIR.
...
\par It turns out that using single frequency waveform with the feedback beamformer is impractical, being too sensitive to even mild target range estimation errors.
\par Fortunately, using two frequency waveform and applying simple frequency domain manipulations to the output and feedback signals, were found to serve as a low frequency (hence low sensitivity) equivalent of the single frequency scheme.
...
\par Future study of the feedback beamformer concept may inspect other interesting choices of coefficient rather than the DS generalization, suggest other waveforms and associated processing schemes, extend the results to dynamic/multiple targets etc.
...
% In this contribution we have presented a novel concept of spatial feedback-based beamforming. In the context of localization with ULAs, this approach may be considered to be an extension of FIR, to IIR filter design.
...
% \par The suggested approach revealed itself to be sensitive to target range errors. Hence, we have suggested to mitigate this sensitivity by using two independent and closely spaced frequencies.
