...
the output signal ($z$) is synthesized using $\vBeta$, $\vAlpha$ synthesize the feedback transmission ($\text{Tx}$) and $s$ is the input waveform. 
An additive noise model (n), is assumed at the array's output. 
...
Following previous observations, we use the $\coefSetName$ beamformer for both feedback and output signals synthesis, setting
...
\end{equation}
is denoted to serve as the estimated steering vector and $\hat{g},\hat{\phi},\hat{\theta}$ are the gain, range-related phase and the DOA-related phase estimations respectively.
...
\begin{equation}
    ...
    \begin{split}
        ...
        H_{\vecnot{\beta}_{\text{opt}},\vecnot{\alpha}_{\text{opt}}}\omegaB
        }
         =
        \frac{
        \HbaFB
...
\]
Also, $\HrTPr$'s subscript emphasize its DOA, range and gain errors dependency.
...
$$
Next, we evaluate the FB's beamwidth, sidelobe level and directivity and compare them to standard ULA (i.e. $r=0$) considering the steer error scenario (i.e. $\dPhi=0$) where 
...
\ifdefined\showDev
    ...
    % \resizebox{0.9\linewidth}{!}{
        \begin{split}
            &\frac{\partial}{\partial\dTheta}\Hr_{\Delta\theta,\Delta\phi=0,r=0}\omegaB &\\
...
\par Finally, expressing the improvement in directivity compared to the classical ULA, assuming large $N$ values, gives rise to
