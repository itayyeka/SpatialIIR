...
\par Following previous observations, we analyze the case of CB (i.e. coherently sum the array elements) using
...
\end{equation}
as the steering vector estimation and $\hat{\phi}, \hat\theta$ are the range-related phase and the DOA-related phase estimations respectively.
\par Plugging \eqref{eq:alpha_beta_hat} within \eqref{eqn:GeneralFeedbackTransferFunction}, we obtain
...
\]
respectively. 
\par We then define four fundamental scenarios:
\begin{itemize}
...
\par We now focus on the steer error scenario, i.e. $\dPhi=0$, and compare the beamwidth, sidelobe level and directivity between  
...
Another common measure for the array performance is its directivity \cite{VanTrees2002DetectionIV}, $\mathrm{D}$, defined as
\begin{equation}\label{eq_D}
    \mathrm{D}(r) = \frac{\Hr_{\dTheta=0,\dPhi=0,r}}{\frac{1}{2\pi}\int_{0}^{2\pi}\Hr_{\dTheta,\dPhi=0,r}\ d\dTheta}.
\end{equation}
It measures the ratio between the maximal array gain at its mainlobe, to the averaged gain over all directions. For uniformly weighted ULAs with no feedback, it is known \cite{VanTrees2002DetectionIV} that $\mathrm{D}(r=0) = N$.
