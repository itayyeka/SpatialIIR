...
As commonly done for ULA parameter analysis \cite{van2004optimum}, we focus on the normalized response (i.e. where the peak main lobe gain is set to $0_{dB}$). Hence forth, we define the normalized pattern
\begin{equation}
    ...
    %}
\end{equation}
where $\vecnot{\beta}_{\coefSetName,\text{opt}}=\vecnot{\alpha}_{\coefSetName,\text{opt}}$ and its subscript (i.e. $\Delta\theta,\Delta\phi,r$) express its DOA, range and gain errors dependency.
Plugging \eqref{eq:SF_CB} to \eqref{eq_narmalized_pattern} gives rise to 
