...
\end{equation}
where $\tau_{n}=n\delta\cos\Brack{\thetaD}/c$ represents the time difference of the arrival between the $n$'th sensor and the reference sensors and $g$ is the attenuation related to both propagation and the target's radar cross section (RCS).
...
\[
\vecnot{\F{x}}\rBrace{\omega}=g\Steer{}\F{x}_{0}\rBrace{\omega}
\]
where $\Steer{}$ denotes the steering vector whose $n$'th element is
\begin{equation}
    \label{eq:d}
    \vecnot{d}_{\vBrace{n}}\rBrace{\omega} = \exp{\rBrace{-j\omega\tau_{n}}},\;n=0,\ldots,N-1.
\end{equation}
The frequency domain output of an $N$ element array beamformer, configured with frequency dependent weights $\vBeta$, is denoted as $Z\rBrace{\omega}$ and may be expressed as
$$ Z\rBrace{\omega} = g\vBetaT\vd\F{x}_{0}\rBrace{\omega}. $$ 
...
\[
Z\rBrace{\omega} = g\F{x}_0\rBrace{\omega}\sum_{n=0}^{N-1}\beta_{n}\rBrace{\omega}\exp\Brack{-jn\theta},
\]
hence in the ULA case, aiming for a desired spatial response, the weights vector $\vBeta$ configuration is mathematically equivalent to FIR filter design~\cite{van1988beamforming}.
