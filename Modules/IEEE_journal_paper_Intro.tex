...
\begin{equation}
x_{n}(t) = g\rBrace{t}*s\Brack{t-\tau_{pd}-\tau_{n}},
\label{eqn:noFeedbackULA_singleSensor_temporal}
\end{equation}
where $\tau_{n}=n\delta\cos\Brack{\thetaD}/c$ represents the time difference of the arrival between the $n$'th sensor and the reference sensor and $g\rBrace{t}$ is the channel's response, related to both propagation and the target's radar cross section (RCS).
...
\[
\vecnot{\F{x}}\rBrace{\omega}=g\rBrace{\omega}\vecnot{d}\rBrace{\omega}\F{s}\rBrace{\omega}
\]
where $g\rBrace{\omega}$ is assumed to be frequency-only dependent for simplicity and $\vdO$ denotes the steering vector whose $n$'th element is
\begin{equation}
    \label{eq:d}
    \vecnot{d}_{\vBrace{n}}\rBrace{\omega} = \exp{\rBrace{-j\omega\tau_{n}}},\;n=0,\ldots,N-1.
\end{equation}
The frequency domain output of an $N$ element array beamformer, configured with frequency dependent weights $\vecnot{\beta}\rBrace{\omega}$, is denoted as $Z\rBrace{\omega}$ and may be expressed as
$$ Z\rBrace{\omega} = g\rBrace{\omega}\vecnot{\beta}^{T}\rBrace{\omega}\vecnot{d}\rBrace{\omega}\F{s}\rBrace{\omega}. $$ 
...
\[
Z\rBrace{\omega} = g\rBrace{\omega}\F{s}\rBrace{\omega}\sum_{n=0}^{N-1}\beta_{n}\rBrace{\omega}\exp\Brack{-jn\theta},
\]
