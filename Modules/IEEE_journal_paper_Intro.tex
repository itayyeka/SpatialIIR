...
\end{equation}
where $\tau_{n}=n\delta\cos\Brack{\thetaD}/c$ represents the time difference of the arrival between the $n$'th sensor and the reference sensor and $g$ is the attenuation related to both propagation and the target's radar cross section (RCS), assumed to be frequency-only dependent for simplicity.
...
\[
\vecnot{\F{x}}\rBrace{\omega}=g\rBrace{\omega}\vecnot{d}\rBrace{\omega}\F{x}_{0}\rBrace{\omega}
\]
where $\vdO$ denotes the steering vector whose $n$'th element is
\begin{equation}
    \label{eq:d}
...
$$ Z\rBrace{\omega} = g\rBrace{\omega}\vecnot{\beta}^{T}\rBrace{\omega}\vecnot{d}\rBrace{\omega}\F{x}_{0}\rBrace{\omega}. $$ 
...
\[
Z\rBrace{\omega} = g\rBrace{\omega}\F{x}_0\rBrace{\omega}\sum_{n=0}^{N-1}\beta_{n}\rBrace{\omega}\exp\Brack{-jn\theta},
\]
