...
Denoting the spatial pointer $\vv{p}$ to be a Cartesian coordinate set, we consider an $N$-element ULA with inter-element spacing $\delta$ where its $n$'th sensor is positioned at $\vv{p_{n}},\;\text{for}\; n\in\vBrace{0,\ldots,N-1}$. 
We set $\vv{p_{0}}$ as the reference point (i.e. all coordinates are zero) and assume that the target of interest is positioned at $\vv{p_{\text{t}}}$.
...
Inspired by radar based applications, we also place a transmitter at $\vv{p_{0}}$, assuming the transmitted signal $s$, reflects back from the target and re-impinges the array, with total time delay of $\tau_{\text{pd}}=2R/c$ seconds, where $R = \norm{\vv{p_{\text{t}}}-\vv{p_{0}}}$ is the target's range and $c$ represents the propagation velocity of the signal in the medium.
In the following, we use some basic conventions; vectors and matrices are denoted by bold italic letters, $v_{i}$ is denoted as the $i$-th element of the vector $\vecnot{v}$ and $A_{ij}$ is the $i,j$-th element of the matrix $\vecnot{A}$.
Also, $\vecnot{A}^{T}$, $\vecnot{A}^{*}$ and $\vecnot{A}^{H}$ are the transpose, conjugate and hermitian conjugate of matrix $\vecnot{A}$ respectively.
