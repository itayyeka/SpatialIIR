...
% Assuming perfect knowledge of the target's location, while aiming to maximize the FIM diagonal elements via denominator (i.e. $\lBrace{1-g\aTd\ePhi{-}}$) minimization, the optimal feedback weights are
% \begin{equation}\label{eq:alpha_beta_opt}
% \vAlpha_{\text{CB,opt}}=\frac{\vdC\exp\rBrace{j\phi}}{\hat{g}\norm{\vd}^2},
% \end{equation}
...
\par Note that setting the feedback weights as in \eqref{eq:alpha_beta_opt} requires a perfect knowledge of the target's location. 
In practice, where the target's exact location is unknown, there will be unavoidable errors.
