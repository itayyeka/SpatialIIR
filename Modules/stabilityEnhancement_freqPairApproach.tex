...
We now propose an architecture which obtains the desired beampattern $\Hr_{\dTheta,\dPhi\to0,r}$ even when in practice, the true range to the target is unknown (i.e. $\Delta R_{\text{rt}}$ is large). Moreover, we intend to show that the suggested architecture operates at moderately low signal to noise ratio (SNR).
...
\par We define the dual frequency (DF) beamformer as the harmonic mean of $H_{\vAlpha_{i},\vBeta_{i}}(\omega_i)$, 
\begin{equation*}
...
may be expressed as 
\begin{equation*}
    \vd_{\thetaD}[n] = g_{n,\thetaD}(\omega)\exp{\rBrace{-j\omega\tau_{n,\thetaD}}},\;n=0,\ldots,N-1.
\end{equation*}
To simplify the exposition, we assume common gain across the array (i.e. $g_{\thetaD}(\omega)$ is not a function of the element index $n$) and suppress the DOA ($\thetaD$) dependency (which is the same at both frequencies) such that one may express the $i$'th frequency steering vector as
...
\par Assume that we aim at maximal phase error of $\Delta \phi=0.01\pi_{\text{RAD}}$. Hence, when using the \textit{dual frequency} architecture, the dictated frequency relation is
...
\par Note that for close frequencies such that $r_{1}\approx{}r_{2}$, $\kappa$ tends towards unity, thus significantly mitigating the gain mismatch effect even when both feedback beamformers are mismatched.
