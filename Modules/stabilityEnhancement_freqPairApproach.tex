...
Motivated to mitigate the $\Delta\phi$ dependency of the system, we exploit the fact that the system response may be easily evaluated via the frequency domain (i.e. $H_{\vBetaI{i}\vAlphaI{i}}\rBrace{\omega_{i}}$ is merely the temporal complex coefficient of $z_i(t)$'s Fourier transform) and define the DF beamformer to be the harmonic mean of both beamformers' outputs (see Fig.~\ref{fig_app}). It follows that
\begin{equation}
    \label{eqn_HDF_def}
...
\end{equation}
where \textit{$\vAlphaI{i},\vBetaI{i}$} are the coefficients of the $i$'th FB, results in 
\begin{equation}
